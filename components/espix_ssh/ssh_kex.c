...
esp_err_t ssh_kex_run(ssh_conn_t *c)
{
    ...
    /*
     * Phase timings, DEBUG. The key exchange spends ~350ms of CPU here, and the
     ...
     * multiplication, on in this build -- is why the signature is ~60ms;
     * verifying it costs two variable-point multiplications and is done once at
     * key load, not per connection. What remains here is X25519, which no such
     * option can help.
     */
